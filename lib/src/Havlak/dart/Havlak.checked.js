...
(function() {
  ...
  var dart = [["_foreign_helper", "dart:_foreign_helper",, H, {
    ...
    BenchmarkBase_measure_closure0: {
      ...
      call$0: function() {
        var t1, t2, numLoops;
        t1 = [A.SimpleLoop];
        t2 = new A.SimpleLoop(H.assertSubtype([], "$isList", [A.BasicBlock], "$asList"), H.assertSubtype([], "$isList", t1, "$asList"), 0, null, null, false, true, 0, 0);
        t1 = H.assertSubtype([], "$isList", t1, "$asList");
    ...
    buildStraight: function(cfg, start, n) {
      ...
      for (t1 = cfg.edgeList, i = 0; i < n; ++i) {
        ...
        t3.to = t2;
        C.JSArray_methods.add$1(t4.outEdges, t2);
        C.JSArray_methods.add$1(t2.inEdges, t4);
    ...
    buildBaseLoop: function(cfg, from) {
      ...
      t1.to = t3;
      C.JSArray_methods.add$1(t2.outEdges, t3);
      C.JSArray_methods.add$1(t3.inEdges, t2);
      t2 = cfg.edgeList;
      ...
      t1.to = t4;
      C.JSArray_methods.add$1(t3.outEdges, t4);
      C.JSArray_methods.add$1(t4.inEdges, t3);
      C.JSArray_methods.add$1(t2, t1);
      t1 = new A.BasicBlockEdge(null, null);
    ...
    BasicBlockEdge: {
      ...
      BasicBlockEdge$3: function(cfg, fromName, toName) {
        ...
        this.to = t1;
        C.JSArray_methods.add$1(this.from.outEdges, t1);
        C.JSArray_methods.add$1(this.to.inEdges, this.from);
        C.JSArray_methods.add$1(cfg.edgeList, this);
      },
      static: {
    ...
    CFG: {
      ...
      createNode$1: function($name) {
        ...
        node = H.interceptedTypeCheck(t1.$index(0, $name), "$isBasicBlock");
        if (node == null) {
          t2 = [A.BasicBlock];
          node = new A.BasicBlock($name, H.assertSubtype([], "$isList", t2, "$asList"), H.assertSubtype([], "$isList", t2, "$asList"));
    ...
    SimpleLoop: {
      ...
      checksum$0: function() {
        ...
        t2 = this.header;
        if (t2 != null)
          t1.result = ((result & 268435455) << 1) + t2.name;
    ...
    SimpleLoop_checksum_closure: {
      ...
      call$1: function(e) {
        var t1, result;
        t1 = this._box_0;
        result = C.JSInt_methods.$add((t1.result & 268435455) << 1, H.intTypeCheck(e.get$name()));
        t1.result = result;
        return result;
      }
    },
    ...
    UnionFindNode: {
      ...
      findSet$0: function() {
        ...
        nodeList = H.assertSubtype([], "$isList", [A.UnionFindNode], "$asList");
        for (node = this; t1 = node.parent, node !== t1;) {
          t2 = t1.parent;
          if (t1 == null ? t2 != null : t1 !== t2)
            C.JSArray_methods.add$1(nodeList, node);
          node = node.parent;
        }
    ...
    HavlakLoopFinder: {
      ...
      DFS$5: function(currentNode, nodes, number, last, current) {
        var t1, t2, t3, lastid, target, t4, t5;
        H.assertSubtype(nodes, "$isList", [A.UnionFindNode], "$asList");
        t1 = [P.int];
        H.assertSubtype(number, "$isList", t1, "$asList");
        H.assertSubtype(last, "$isList", t1, "$asList");
        ...
        number[t1] = current;
        for (t3 = currentNode.outEdges, lastid = current, target = 0; target < t3.length; ++target) {
          t4 = t3[target];
          t5 = t4.name;
          if (t5 >= t2)
            return H.ioore(number, t5);
          if (number[t5] === -1)
            lastid = this.DFS$5(t4, nodes, number, last, lastid + 1);
        }
        C.JSArray_methods.$indexSet(last, number[t1], lastid);
        return lastid;
      },
      findLoops$0: function() {
        var t1, t2, size, nonBackPreds, backPreds, number, header, types, last, nodes, t3, t4, t5, t6, t7, t8, t9, i, w, nodeW, nv, t10, v, t11, nodePool, vi, t12, workList, n, x, t13, iter, ydash, t14, loop, np, node;
        t1 = this.cfg;
        t2 = t1.startNode;
        if (t2 == null)
          return 0;
        ...
        t1 = [[P.List, P.int]];
        nonBackPreds = H.assertSubtype(new Array(size), "$isList", t1, "$asList");
        backPreds = H.assertSubtype(new Array(size), "$isList", t1, "$asList");
        t1 = [P.int];
        number = H.assertSubtype(new Array(size), "$isList", t1, "$asList");
        header = H.assertSubtype(new Array(size), "$isList", t1, "$asList");
        types = H.assertSubtype(new Array(size), "$isList", t1, "$asList");
        last = H.assertSubtype(new Array(size), "$isList", t1, "$asList");
        t1 = [A.UnionFindNode];
        nodes = H.assertSubtype(new Array(size), "$isList", t1, "$asList");
        for (t3 = nonBackPreds.length, t4 = backPreds.length, t5 = number.length, t6 = header.length, t7 = types.length, t8 = last.length, t9 = nodes.length, i = 0; i < size; ++i) {
          if (i >= t3)
            return H.ioore(nonBackPreds, i);
          nonBackPreds[i] = [];
          if (i >= t4)
            return H.ioore(backPreds, i);
          backPreds[i] = [];
          if (i >= t5)
            return H.ioore(number, i);
          number[i] = -1;
          if (i >= t6)
            return H.ioore(header, i);
          header[i] = 0;
          if (i >= t7)
            return H.ioore(types, i);
          types[i] = 1;
          if (i >= t8)
            return H.ioore(last, i);
          last[i] = 0;
          if (i >= t9)
        ...
        for (w = 0; w < size; ++w) {
          if (w >= t9)
            return H.ioore(nodes, w);
          nodeW = nodes[w].bb;
          if (nodeW == null) {
            if (w >= t7)
              return H.ioore(types, w);
            types[w] = 5;
          } else {
            t2 = nodeW.inEdges;
            if (t2.length > 0)
              for (nv = 0; nv < t2.length; ++nv) {
                t10 = t2[nv].name;
                if (t10 >= t5)
                  return H.ioore(number, t10);
                v = number[t10];
                if (v !== -1) {
                  if (C.JSInt_methods.$le(w, v)) {
                    if (w >= t8)
                      return H.ioore(last, w);
                    t10 = last[w];
                    if (typeof v !== "number")
                      return v.$le();
                    t10 = C.JSInt_methods.$le(v, t10);
                  } else
                    t10 = false;
                  if (t10) {
                    if (w >= t4)
                      return H.ioore(backPreds, w);
                    t10 = backPreds[w];
                    (t10 && C.JSArray_methods).add$1(t10, v);
                  } else {
                    if (w >= t3)
                      return H.ioore(nonBackPreds, w);
                    t10 = nonBackPreds[w];
                    (t10 && C.JSArray_methods).add$1(t10, v);
                  }
                }
        ...
        for (w = size - 1, t2 = this.lsg, t5 = t2.loops, t10 = [A.BasicBlock], t11 = [A.SimpleLoop]; w >= 0; --w) {
          nodePool = H.assertSubtype([], "$isList", t1, "$asList");
          if (w >= t9)
            return H.ioore(nodes, w);
          nodeW = nodes[w].bb;
          if (nodeW == null)
          ...
          if (w >= t4)
            return H.ioore(backPreds, w);
          vi = 0;
          for (; t12 = backPreds[w], vi < t12.length; ++vi) {
            v = t12[vi];
            if (v !== w)
              C.JSArray_methods.add$1(nodePool, C.JSArray_methods.$index(nodes, v).findSet$0());
            else {
              if (w >= t7)
                return H.ioore(types, w);
              types[w] = 3;
          ...
          workList = H.assertSubtype([], "$isList", t1, "$asList");
          for (n = 0; t12 = nodePool.length, n < t12; ++n)
            C.JSArray_methods.add$1(workList, nodePool[n]);
          if (t12 !== 0) {
            if (w >= t7)
              return H.ioore(types, w);
            types[w] = 2;
          }
          for (t12 = H.getTypeArgumentByIndex(workList, 0); workList.length > 0;) {
            x = H.interceptedTypeCheck(H.assertSubtypeOfRuntimeType(workList.splice(0, 1)[0], t12), "$isUnionFindNode");
            t13 = x.dfsNumber;
            if (t13 >= t3)
              return H.ioore(nonBackPreds, t13);
            if (nonBackPreds[t13].length > 32768)
              return 0;
            iter = 0;
            while (true) {
              t13 = x.dfsNumber;
              if (t13 >= t3)
                return H.ioore(nonBackPreds, t13);
              t13 = nonBackPreds[t13];
              if (!(iter < t13.length))
                break;
              ydash = C.JSArray_methods.$index(nodes, t13[iter]).findSet$0();
              t13 = ydash.dfsNumber;
              if (w <= t13) {
                if (w >= t8)
                  return H.ioore(last, w);
                t13 = C.JSInt_methods.$le(t13, last[w]);
              } else
                t13 = false;
              if (!t13) {
                if (w >= t7)
                ...
                if (w >= t3)
                  return H.ioore(nonBackPreds, w);
                t13 = nonBackPreds[w];
                (t13 && C.JSArray_methods).add$1(t13, ydash.dfsNumber);
              } else if (ydash.dfsNumber !== w)
                if (C.JSArray_methods.indexOf$1(nodePool, ydash) === -1) {
                  C.JSArray_methods.add$1(workList, ydash);
                  C.JSArray_methods.add$1(nodePool, ydash);
                }
              ++iter;
            }
          }
          ...
          if (t12) {
            t12 = t2.loopCounter++;
            t13 = H.assertSubtype([], "$isList", t10, "$asList");
            t14 = H.assertSubtype([], "$isList", t11, "$asList");
            loop = new A.SimpleLoop(t13, t14, t12, null, null, false, true, 0, 0);
            C.JSArray_methods.add$1(t13, nodeW);
            loop.header = nodeW;
            if (w >= t7)
              return H.ioore(types, w);
            if (!(types[w] === 4))
            ...
            for (np = 0; np < nodePool.length; ++np) {
              ...
              t12 = node.dfsNumber;
              if (t12 >= t6)
                return H.ioore(header, t12);
              header[t12] = w;
              node.parent = nodes[w];
              t12 = node.loop;
              if (t12 != null) {
                t12.parent = loop;
                C.JSArray_methods.add$1(t14, t12);
              } else
                C.JSArray_methods.add$1(t13, node.bb);
            }
            C.JSArray_methods.add$1(t5, loop);
          }
        }
        return t5.length;
      }
    ...
    Havlak: {
      ...
      warmup$0: function() {
        var t1, t2, t3, dummyloop, t4, t5, lsg, checksum;
        for (t1 = this.cfg, t2 = [A.BasicBlock], t3 = [A.SimpleLoop], dummyloop = 0; dummyloop < 20; ++dummyloop) {
          t4 = new A.SimpleLoop(H.assertSubtype([], "$isList", t2, "$asList"), H.assertSubtype([], "$isList", t3, "$asList"), 0, null, null, false, true, 0, 0);
          t5 = H.assertSubtype([], "$isList", t3, "$asList");
          lsg = new A.LSG(1, t5, t4);
          t4.isRoot = true;
          C.JSArray_methods.add$1(t5, t4);
          new A.HavlakLoopFinder(t1, lsg).findLoops$0();
          checksum = lsg.checksum$0();
          if (checksum !== 435630002)
            throw H.wrapException("Wrong checksum - expected <435630002>, but was <" + checksum + ">");
        }
      },
      Havlak$0: function() {
        ...
        for (t2 = t1.edgeList, n = 2, parlooptrees = 0; parlooptrees < 10; ++parlooptrees) {
          ...
          t3.to = t5;
          C.JSArray_methods.add$1(t4.outEdges, t5);
          C.JSArray_methods.add$1(t5.inEdges, t4);
      ...
      static: {
        Havlak$: function() {
          var t1 = new A.Havlak(new A.CFG(H.assertSubtype(P.HashMap_HashMap(null, null, null, null, null), "$isMap", [P.int, A.BasicBlock], "$asMap"), H.assertSubtype([], "$isList", [A.BasicBlockEdge], "$asList"), null), "Havlak");
          t1.Havlak$0();
          return t1;
        }
      }
  ...
  $ = new Isolate();
  init.metadata = [];
  init.types = [{func: 1}, {func: 1, args: [,]}, {func: 1, ret: P.num}];
...
//# sourceMappingURL=Havlak.checked.js.map
